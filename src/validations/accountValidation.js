...
const createAccount = async (req, res, next) => {
  const checkEmpty = Joi.object({
    ...
    fullname: Joi.string().required().max(50).trim().strict().messages({
...
const updateAccount = async (req, res, next) => {
  const checkUpdate = Joi.object({
    ...
    password: Joi.string().optional().min(6).max(50).trim().strict().messages({
      "string.min": "Mật khẩu phải có ít nhất 6 ký tự",
    }),
...
const resetPassword = async (req, res, next) => {
  ...
  try {
    await checkResetPassword.validateAsync(req.body, { abortEarly: false });

    const { email, phonenumber } = req.body;
    const account = await Account.findOne({ email, phonenumber });
